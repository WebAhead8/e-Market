...
function MarketList({ maxPrice, minPrice, items, search, setItems, catFilter }) {
	const addItem = (markt) => {
		setItems(items.concat(markt));
	...
	React.useEffect(
		() => {
			const result = searchResult.filter((item) => item.name.toLowerCase().startsWith(search));
