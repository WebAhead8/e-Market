...
<<<<<<< HEAD
    ...
    const productsToShow = Object.keys(listOfProducts).map((product) => {
       return <tr> <td>{product}</td> <td>{listOfProducts[product].quantity}</td> <td>{listOfProducts[product].price}</td> </tr>;
    })
    ...
    return (
        <div>
            <table>
                <tr> <th>Product Name</th> <th>Quantity</th> <th>Price Of One Product</th> </tr>
                {productsToShow}
            </table>
